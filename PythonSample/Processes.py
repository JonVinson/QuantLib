...
def gaussian(n_paths, n_steps, mu, sig):
...
def sabr(n_paths, n_steps, mu = 0.0, alpha = 0.0, beta = 1.0, rho = 0.0, f0 = 0.0, sig0 = 0.0, dt = 1.0):
    ...
    # (in case values need to be recalculated)
    ...
    for i in range(n_steps):
        f[:, i + 1] = f[:, i] + sig[:, i] * np.power(f[:, i], beta) * z2[:, i]
